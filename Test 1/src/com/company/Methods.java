...
public class Methods implements StudentEnrollmentManager {
    ...
    public void AddEnrollment(List<Student> studentsList, List<Course> courseList) {
        ...
        if (checkduplicated.Checkduplicated(sid, cid, sem)){        // check duplicate
            return;     // return main menu when found a duplicate
        }else{
        studentEnrollmentList.add(new StudentEnrollment(sid, cid, sem)); // if all three input correct and no duplicate in the list, add new enrollment to the student enrollment list
        }
            ...
            switch (option) {
                case "1" -> {       // add method
                    ...
                    if (checkduplicated.Checkduplicated(sid, ucid, usem)){
                        break;      // when the program found a duplicate in the list, the program will return to the update menu
                    }else{
